"""
Script para testar a emissão de nota fiscal com nova configuração de RPS
Série: 1
Número: Será obtido da configuração atual
"""
import os
import sys
import django
...
def get_current_rps_config():
    """
    Obtém a configuração atual de RPS do professor
    """
    try:
...
if __name__ == "__main__":
    logger.info("=== Teste de Emissão de Nota Fiscal com Nova Série RPS ===")
    
    # Obter configuração atual
    config = get_current_rps_config()
    ...
    if config:
        ...
        logger.error("Não foi possível obter a configuração de RPS.")
